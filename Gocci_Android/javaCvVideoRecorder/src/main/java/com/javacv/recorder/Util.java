...
public class Util {
	...
	public static String getRecordingTimeFromMillis(long millis)
	{
        ...
        //撮ったカメラの向きを横から縦へ・・・http://dai1741.hatenablog.com/entry/2012/02/15/011114
	...
	public static String createFinalPath()
	{
	    ...
	    final String filename = title + CONSTANTS.VIDEO_EXTENSION;
		final String filePath = genrateFilePath(String.valueOf(dateTaken), true, null);
	...
	public static String genrateFilePath(String uniqueId, boolean isFinalPath, File tempFolderPath)
	{
    ...
    //画質の変更をしているよう。不要なので消したい。常にHIGH_RESOLUTION
	...
	public static int calculateMarginWidth(int previewWidth, int screenWidth, int previewHeight, int screenHeight)
	{
		int margin = 0;
		if(previewWidth <= CONSTANTS.RESOLUTION_LOW && previewHeight <= CONSTANTS.RESOLUTION_LOW)
		{
			margin = (int) (screenHeight*0.2);
		}
		else if(previewWidth > CONSTANTS.RESOLUTION_LOW && previewWidth <= CONSTANTS.RESOLUTION_MEDIUM
                && previewHeight > CONSTANTS.RESOLUTION_LOW && previewHeight <= CONSTANTS.RESOLUTION_MEDIUM)
		{
			margin = (int) (screenHeight*0.2);
		}
		else if(previewWidth > CONSTANTS.RESOLUTION_MEDIUM && previewWidth <= CONSTANTS.RESOLUTION_HIGH
                && previewHeight > CONSTANTS.RESOLUTION_MEDIUM && previewHeight <= CONSTANTS.RESOLUTION_HIGH)
		{
			margin = (int) (screenHeight*0.2);
		}
