...
@interface CollectionViewCell_2()
...
{
    if ([self.delegate respondsToSelector:@selector(collection:didTapLikeButton:)]) {
        ...
        if(flash_on == 0 ){
            ...
            dispatch_sync(globalQueue, ^{
                [self.delegate collection:self didTapLikeButton:self.postID];
            });
        }else{
