...
NSString * const LifelogCellIdentifier = @"LifelogCell";

@interface LifelogCell()
...
{
    if ([self.delegate respondsToSelector:@selector(lifelogCell:didTapNameWithUserName:picture:)]) {
        [self.delegate lifelogCell:self didTapNameWithUserName:self.username picture:_userspicture];
    }
}

- (void)tapthumb:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(lifelogCell:didTapthumb:)]) {
        [self.delegate lifelogCell:self didTapthumb:self.thumbnailView];
    }
}

- (void)tapAvaterImageView:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(lifelogCell:didTapNameWithUserPicture:name:)]) {
        [self.delegate lifelogCell:self didTapNameWithUserPicture:self.userspicture name:_username];
    }
}

- (void)tapRestautant:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(lifelogCell:didTapRestaurant:locality:tel:homepage:category:)]) {
        [self.delegate lifelogCell:self didTapRestaurant:self.restname locality:self.locality tel:self.tell homepage:self.homepage category:self.category];
    }
}

- (void)tapLike:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(lifelogCell:didTapLikeButtonWithPostID:)]) {
        [self.delegate lifelogCell:self didTapLikeButtonWithPostID:self.postID];
    }
}

- (void)tapNavi:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(lifelogCell:didTapNaviWithLocality:)]) {
        [self.delegate lifelogCell:self didTapNaviWithLocality:self.locality];
    }
}

- (void)tapComment:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(lifelogCell:didTapCommentButtonWithPostID:)]) {
        [self.delegate lifelogCell:self didTapCommentButtonWithPostID:self.postID];
    }
}
...
{
    if ([self.delegate respondsToSelector:@selector(lifelogCell:didTapViolateButtonWithPostID:)]) {
        [self.delegate lifelogCell:self didTapViolateButtonWithPostID:self.postID];
    }
}

- (void)onDeleteButton:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(lifelogCell:didTapDeleteWithPostID:)]) {
        [self.delegate lifelogCell:self didTapDeleteWithPostID:self.postID];
    }
}
...
{
    LifelogCell *cell = [LifelogCell cell];
