...
{
    if ([self.delegate respondsToSelector:@selector(followViewCell:didTapLikeButton:tapped:)]) {
        ...
        if(flash_on == 0 ){
            ...
            dispatch_sync(globalQueue, ^{
                [self.delegate followViewCell:self didTapLikeButton:self.postID tapped:YES];
            });
        }else{
            UIImage *img = [UIImage imageNamed:@"Likes_off.png"];
            [_likeBtn setBackgroundImage:img forState:UIControlStateNormal];
            ...
            dispatch_queue_t globalQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0);
            dispatch_sync(globalQueue, ^{
                [self.delegate followViewCell:self didTapLikeButton:self.postID tapped:NO];
            });
        }
