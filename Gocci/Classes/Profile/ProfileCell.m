...
+ (instancetype)cell
{
    return [[NSBundle mainBundle] loadNibNamed:@"ProfileCell" owner:self options:nil][0];
}
...
{
    if ([self.delegate respondsToSelector:@selector(profileCell:didTapNameWithUserName:picture:)]) {
        [self.delegate profileCell:self didTapNameWithUserName:self.username picture:_userspicture];
    }
}

- (void)tapthumb:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell:didTapthumb:)]) {
        [self.delegate profileCell:self didTapthumb:self.thumbnailView];
    }
}

- (void)tapAvaterImageView:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell:didTapNameWithUserPicture:name:)]) {
        [self.delegate profileCell:self didTapNameWithUserPicture:self.userspicture name:_username];
    }
}

- (void)tapRestautant:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell:didTapRestaurant:locality:tel:homepage:category:)]) {
        [self.delegate profileCell:self didTapRestaurant:self.restname locality:self.locality tel:self.tell homepage:self.homepage category:self.category];
    }
}

- (void)tapLike:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell:didTapLikeButtonWithPostID:)]) {
        [self.delegate profileCell:self didTapLikeButtonWithPostID:self.postID];
    }
}

- (void)tapNavi:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell:didTapNaviWithLocality:)]) {
        [self.delegate profileCell:self didTapNaviWithLocality:self.locality];
    }
}

- (void)tapComment:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell:didTapCommentButtonWithPostID:)]) {
        [self.delegate profileCell:self didTapCommentButtonWithPostID:self.postID];
    }
}
...
{
    if ([self.delegate respondsToSelector:@selector(profileCell:didTapViolateButtonWithPostID:)]) {
        [self.delegate profileCell:self didTapViolateButtonWithPostID:self.postID];
    }
}

- (void)onDeleteButton:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell:didTapDeleteWithPostID:)]) {
        [self.delegate profileCell:self didTapDeleteWithPostID:self.postID];
    }
}
...
{
    ProfileCell *cell = [ProfileCell cell];
