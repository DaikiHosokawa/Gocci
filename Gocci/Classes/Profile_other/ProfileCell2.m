...
NSString * const Profile2CellIdentifier = @"ProfileCell2";

@interface ProfileCell2()
...
+ (instancetype)cell
{
    return [[NSBundle mainBundle] loadNibNamed:@"ProfileCell2" owner:self options:nil][0];
}
...
{
    if ([self.delegate respondsToSelector:@selector(profileCell2:didTapNameWithUserName:picture:)]) {
        [self.delegate profileCell2:self didTapNameWithUserName:self.username picture:_userspicture];
    }
}

- (void)tapthumb:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell2:didTapthumb:)]) {
        [self.delegate profileCell2:self didTapthumb:self.thumbnailView];
    }
}

- (void)tapAvaterImageView:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell2:didTapNameWithUserPicture:name:)]) {
        [self.delegate profileCell2:self didTapNameWithUserPicture:self.userspicture name:_username];
    }
}

- (void)tapRestautant:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell2:locality:tel:homepage:category:)]) {
        [self.delegate profileCell2:self didTapRestaurant:self.restname locality:self.locality tel:self.tell homepage:self.homepage category:self.category];
    }
}

- (void)tapLike:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell2:didTapLikeButtonWithPostID:)]) {
        [self.delegate profileCell2:self didTapLikeButtonWithPostID:self.postID];
    }
}

- (void)tapNavi:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell2:didTapNaviWithLocality:)]) {
        [self.delegate profileCell2:self didTapNaviWithLocality:self.locality];
    }
}

- (void)tapComment:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell2:didTapCommentButtonWithPostID:)]) {
        [self.delegate profileCell2:self didTapCommentButtonWithPostID:self.postID];
    }
}
...
{
    if ([self.delegate respondsToSelector:@selector(profileCell2:didTapViolateButtonWithPostID:)]) {
        [self.delegate profileCell2:self didTapViolateButtonWithPostID:self.postID];
    }
}

- (void)onDeleteButton:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(profileCell2:didTapDeleteWithPostID:)]) {
        [self.delegate profileCell2:self didTapDeleteWithPostID:self.postID];
    }
}
...
{
    ProfileCell2 *cell = [ProfileCell2 cell];
