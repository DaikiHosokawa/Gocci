...
CGFloat const kTWMessageViewBarPadding = 7.0f;
CGFloat const kTWMessageViewIconSize = 32.0f;
...
+ (void)initialize
{
	...
	{
        // Fonts
        kTWMessageViewTitleFont = [UIFont boldSystemFontOfSize:14.0];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([self.delegate respondsToSelector:@selector(styleSheetForMessageView:)])
    {
        ...
        CGContextSaveGState(context);
        {
            ...
            {
                [[styleSheet iconImageForMessageType:self.messageType] drawInRect:CGRectMake(xOffset+5, yOffset, kTWMessageViewIconSize, kTWMessageViewIconSize)];
            }
        }
        CGContextRestoreGState(context);
        ...
        if ([[UIDevice currentDevice] isRunningiOS7OrLater])
        {
            ...
            [[self titleColor] set];
            [self.titleString drawWithRect:CGRectMake(xOffset+10, yOffset, titleLabelSize.width, titleLabelSize.height)
            ...
            [self.descriptionString drawWithRect:CGRectMake(xOffset+10, yOffset, descriptionLabelSize.width, descriptionLabelSize.height)
                                         options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingTruncatesLastVisibleLine
...
+ (void)initialize
{
	if (self == [TWDefaultMessageBarStyleSheet class])
	{
        ...
        //(1)
        kTWDefaultMessageBarStyleSheetSuccessBackgroundColor = [UIColor colorWithRed:236./255. green:55./255. blue:54./255. alpha:1.];; // green
